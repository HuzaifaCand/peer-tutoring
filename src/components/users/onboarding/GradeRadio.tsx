...
export const radioClass =
  "flex items-center gap-1 sm:gap-2 w-1/2 bg-hoverBg text-textWhite/90 border border-elevatedBg/20 rounded px-3 py-1 cursor-pointer relative";
  ...
  return (
    <div className="flex flex-col mb-2 w-full">
      ...
      <div className="flex gap-2">
        {["AS", "A2"].map((grade) => (
          <label key={grade} className={clsx(radioClass, "text-xs sm:text-sm")}>
            <input
              type="radio"
              value={grade}
              {...register("grade")}
